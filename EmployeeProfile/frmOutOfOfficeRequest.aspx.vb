...
Partial Class EmployeeProfile_frmOutOfOfficeRequest
    ...
    Dim OutOfOfficeReqData As New clsOutOfOfficeReqData()

    Protected Sub btnSubmit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubmit.Click
        Try
            If Convert.ToDateTime(txtDateFrom.Text) < DateAdd(DateInterval.Month, -14, Now.Date) Or Convert.ToDateTime(txtDateTo.Text) < DateAdd(DateInterval.Month, -14, Now.Date) Then
                Response.Write("You Can't Apply For a Out Of Office Request Older Than One Month.")
                Exit Sub
            End If

            If Convert.ToDateTime(txtDateFrom.Text) > Convert.ToDateTime(txtDateTo.Text) Then
                MessageBox("Select Correct Date Range (Date From <= Date To)")
                txtDateFrom.Text = ""
                txtDateTo.Text = ""
                Exit Sub
            End If
            ...
            OutOfOffReq.DateFrom = Convert.ToDateTime(txtDateFrom.Text)
            OutOfOffReq.DateTo = Convert.ToDateTime(txtDateTo.Text)
            ...
            If Check = 1 Then
                ...
                ShowOutOfOfficeReq(Session("UniqueUserID"))
                ClearReq()
            Else
